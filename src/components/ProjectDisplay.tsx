  ...
  return (
    ...
    >
      <div className={`flex flex-col md:flex-row ${index % 2 === 0 ? 'md:flex-row-reverse' : ''}`}>
        <img src={item.image} alt={item.title} className='general-hover w-full md:w-[70%] shadow-black shadow-lg max-md:mb-3 rounded-xl' />
            ...
            {item.technologies.map((tech, techIndex) => (
              <div key={techIndex} className='flex flex-col items-center h-full'>
                <div className='flex justify-center items-center w-9 h-9'>
                  <img src={tech.image} alt={tech.name} className='w-9 max-md:w-7' />
                </div>
